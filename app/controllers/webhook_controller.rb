        ...
        when Line::Bot::Event::MessageType::Text
          ...
          if @@user_data.has_key?(userId) then
            if @@user_data[userId].has_key?(:location) then
              ...
              uri = URI.parse(CALILAPI_ENDPOINT + "/check?appkey=#{calil_appkey}&isbn=#{books_data.map{|row| row[0]}.join(',')}&systemid=#{library_data.map{|row| row[0]}.join(',')}&format=json&callback=dd")
              message = {
                type: 'text',
                text: uri
              }
